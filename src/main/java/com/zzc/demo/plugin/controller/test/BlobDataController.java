...
public class BlobDataController extends BaseController {
	/**
	 * Logger for this class
	 */
	private static final Logger logger = Logger.getLogger(BlobDataController.class);

	@Autowired
	private ZZCBlobDataServiceI jeecgBlobDataService;
	@Autowired
	...
	public ModelAndView jeecgBlobData(HttpServletRequest request) {
		return new ModelAndView("zzc/demo/test/jeecgBlobDataList");
	}
	...
	public void datagrid(BlobDataEntity jeecgBlobData,HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
		CriteriaQuery cq = new CriteriaQuery(BlobDataEntity.class, dataGrid);
		//查询条件组装器
		com.zzc.core.extend.hqlsearch.HqlGenerateUtil.installHql(cq, jeecgBlobData);
	...
	@ResponseBody
	public AjaxJson del(BlobDataEntity jeecgBlobData, HttpServletRequest request) {
		AjaxJson j = new AjaxJson();
		jeecgBlobData = systemService.getEntity(BlobDataEntity.class, jeecgBlobData.getId());
		message = "删除成功";
	...
	public void exportXls(HttpServletRequest request, String fileId, HttpServletResponse response) {
		// 从数据库取得数据
		BlobDataEntity obj = systemService.getEntity(BlobDataEntity.class, fileId);
	    try {      
	...
	@ResponseBody
	public AjaxJson save(BlobDataEntity jeecgBlobData, HttpServletRequest request) {
		AjaxJson j = new AjaxJson();
		if (StringUtil.isNotEmpty(jeecgBlobData.getId())) {
			message = "更新成功";
			BlobDataEntity t = jeecgBlobDataService.get(BlobDataEntity.class, jeecgBlobData.getId());
			try {
	...
	public ModelAndView addorupdate(BlobDataEntity jeecgBlobData, HttpServletRequest req) {
		if (StringUtil.isNotEmpty(jeecgBlobData.getId())) {
			jeecgBlobData = jeecgBlobDataService.getEntity(BlobDataEntity.class, jeecgBlobData.getId());
			req.setAttribute("jeecgBlobDataPage", jeecgBlobData);
		}
		return new ModelAndView("zzc/demo/test/jeecgBlobData");
	}
}
