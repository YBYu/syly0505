...
import com.zzc.core.constant.DataBaseConstant;
import com.zzc.web.scenicmanage.entity.ScenicData;
import com.zzc.web.sylyUtils.GlobalParams;
import com.zzc.web.system.manager.ClientManager;
import com.zzc.web.system.pojo.base.TSRoleFunction;
...
public class ResourceUtil {
	...
	public static final void selectArea(ScenicData scenicData){
		String roleName = getSessionUserName().getUserKey();
		if(roleName.indexOf(GlobalParams.ROLE_SHIXIAQU_NAME)!=-1){
			scenicData.setArea("0");
		}
		if(roleName.indexOf(GlobalParams.ROLE_YAZHOEUQU_NAME)!=-1){
			scenicData.setArea("1");
		}
		if(roleName.indexOf(GlobalParams.ROLE_HAIDANGQU_NAME)!=-1){
			scenicData.setArea("2");
		}
		if(roleName.indexOf(GlobalParams.ROLE_TIANYAQU_NAME)!=-1){
			scenicData.setArea("3");
		}
		if(roleName.indexOf(GlobalParams.ROLE_JIYANGQU_NAME)!=-1){
			scenicData.setArea("4");
		}
		
	}
	public static final void selectAreaDatagrid(ScenicData scenicData,CriteriaQuery cq ){
		selectAreaDatagrid("area", cq);
		
	}
	public static final void selectAreaDatagrid(CriteriaQuery cq ){
		selectAreaDatagrid("area", cq);
		
	}
	public static final void selectAreaDatagrid(String AreaName,CriteriaQuery cq ){
		String roleName = getSessionUserName().getUserKey();
		if(roleName.indexOf(GlobalParams.ROLE_SHIXIAQU_NAME)!=-1){
			cq.add(Restrictions.eq(AreaName,"0"));
		}
		if(roleName.indexOf(GlobalParams.ROLE_YAZHOEUQU_NAME)!=-1){
			cq.add(Restrictions.eq(AreaName,"1"));
		}
		if(roleName.indexOf(GlobalParams.ROLE_HAIDANGQU_NAME)!=-1){
			cq.add(Restrictions.eq(AreaName,"2"));
		}
		if(roleName.indexOf(GlobalParams.ROLE_TIANYAQU_NAME)!=-1){
			cq.add(Restrictions.eq(AreaName,"3"));		}
		if(roleName.indexOf(GlobalParams.ROLE_JIYANGQU_NAME)!=-1){
			cq.add(Restrictions.eq(AreaName,"4"));
		}
...
//				|| key.equals(DataBaseConstant.UPDATE_NAME)
	...
	public static void main(String[] args) {
		com.zzc.core.util.LogUtil.info(getPorjectPath());
