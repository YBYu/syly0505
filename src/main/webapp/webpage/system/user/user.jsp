...
<t:formvalid formid="formobj" dialog="true" usePlugin="password" layout="table" action="userController.do?saveUser" beforeSubmit="setOrgIds">
	...
	<table style="width: 600px;" cellpadding="0" cellspacing="1" class="formtable">
		...
		<input id="orgIds" name="orgIds" value="8a8ab0b246dc81120146dc8180a20016" type="hidden">
		<%-- <tr>
		...
		<tr id="arearow">
			<td align="right"><label class="Validform_label"> 所属地区: </label></td>
			<td class="value">
				<select name="area" style="width:208px;">   
			    <option value="0" <c:if test="${'0' eq user.area}">selected</c:if>>市辖区</option>   
			    <option value="1" <c:if test="${'1' eq user.area}">selected</c:if>>崖州区</option>   
			    <option value="2" <c:if test="${'2' eq user.area}">selected</c:if>>海棠区</option>   
			    <option value="3" <c:if test="${'3' eq user.area}">selected</c:if>>天涯区</option>   
			    <option value="4" <c:if test="${'4' eq user.area}">selected</c:if>>吉阳区</option>
			    </select>
            </td>
		</tr>
	</table>
...
<script>
	var roleOptions = $("input[name='roleName']");
	var areaOptions = $("select[name='area']");
	var areaRow = $("#arearow");
	window.setInterval(function(){
		if(roleOptions.val() == "超级管理员" || roleOptions.val() == "系统管理员"){
			areaOptions.val("0");
			areaRow.hide();
		}else{
			areaRow.show();
